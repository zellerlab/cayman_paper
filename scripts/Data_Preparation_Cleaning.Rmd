...
rel_abun_conversion <- function(x){
  x <- x %>% filter(feature != "unannotated") %>% mutate_if(is.numeric, ~./max(.)) %>% filter(feature != "category") ## Should doublecheck if everything has a category feature in the datasets. Note that this is no longer required since transforming krazy output to RPKM values. Please be careful that this is not run anywhere in the scripts anymore, since that would massively mess up the analyses
}
...
```{r}
#nonWestern_file_list <- dir_ls("../CAZy_profiles_krazy_2.14.0/Non_Western/")
