...
#permutest(betadisper_test) ##p=0.001 Means we reject that groups have same dispersions
#set.seed(1000) ## To ensure reproducible results
...
pcoa_bray <- ordinate(westernized_non_westernized_ps, method='PCoA', distance = 'bray')
pcoa_plot_study_colored_bray <- plot_ordination(westernized_non_westernized_ps, pcoa_bray, color="study_name_W_NW_info_n", title='PCoA (Bray-Curtis)') + theme_publication() + geom_point(size = 0.1, alpha = 0.05) + scale_color_manual(values=mycolors) + labs(color="Study") +  xlab("PC1 [39.1%]") + ylab("PC2 [15.0%]") + guides(color=guide_legend(ncol = 1)) + theme(legend.position = "none")

## Manhattan
westernized_non_westernized_ps_log10 <- microbiome::transform(westernized_non_westernized_ps, "log10") ## Obtain log10 abundances for Euclidian PCoA
euclidian_log10_matrix <- phyloseq::distance(westernized_non_westernized_ps_log10, method = "euclidean")
betadisper_test <- betadisper(euclidian_log10_matrix, metadata_perm$non_westernized)
permutest(betadisper_test) ##p=0.001 Means we reject that groups have same dispersions
set.seed(1000) ## To ensure reproducible results
adonis_result_euclidian_log10 <- adonis2(euclidian_log10_matrix ~ non_westernized,data = metadata_perm, permutations = 999)

## P-value 0.001 and R2 = 0.11708 

pcoa_euclidian_log10 <- ordinate(westernized_non_westernized_ps_log10, method='PCoA', distance = 'euclidean')
pcoa_plot_euclidian_log10_study_colored <- plot_ordination(westernized_non_westernized_ps_log10, pcoa_euclidian_log10, color="study_name_W_NW_info", title='PCoA (Euclidian log10)') + theme_publication() + geom_point(size = 0.1, alpha = 0.05) + scale_color_manual(values=mycolors) + labs(color="Study") + guides(color=guide_legend(ncol = 1)) + theme(legend.position = "none")
...
Extended_Data_Fig2 <- pcoa_plot_study_colored_bray + pcoa_plot_euclidian_log10_study_colored + plot_layout(design = layout_matrix)

ggsave(here("figures/Extended_Data_Fig2.pdf"), Extended_Data_Fig2, width = 140, height = 150, unit = "mm")
...
Extended_Data_Fig3_1 <- ggplot(CAZyme_count_df %>% mutate(study_name_info_n = as.factor(study_name_info_n)) %>% mutate(study_name_info_n = fct_relevel(study_name_info_n, median_order$study_name_info_n))) +
  #theme_classic() +
  theme_publication() +
...
DF_CAZymes <- glycan_annotations_final_cleaned_long %>% select(Subfamily, FUNCTION_AT_DESTINATION_1) %>% filter(FUNCTION_AT_DESTINATION_1 == "DF") %>% distinct()

CAZyme_count_DF <- W_NW_rel_abun_long %>% filter(OTU %in% DF_CAZymes$Subfamily) %>% filter(Abundance > 1) %>% group_by(Sample) %>% summarise(across(Abundance, ~sum(.x != 0))) %>% rename(Abundance = "Unique_CAZyme_count") ## Note that here it is absolutely crucial to filter on Abundance 1 to get the RPKM filter in there for prevalence estimation. For initial exploration of e.g. read count correlations it isn't really necessary, but for the final figures definitely so.
CAZyme_count_DF_df <- left_join(CAZyme_count_DF, sampledata, by = c("Sample" = "sample_id"))

Extended_Data_Fig3_2 <- ggplot(CAZyme_count_DF_df) +
  theme_publication() +
  aes(x = non_westernized, y = Unique_CAZyme_count, fill = non_westernized) +
  geom_boxplot(outlier.alpha = 0.2) +
  #stat_compare_means(aes(group = non_westernized), label = "p.signif", label.x = 1.5, method = "wilcox.test") +
  #stat_compare_means(aes(group = non_westernized), label = "p.format", label.x = 1.5, method = "wilcox.test") +
  stat_compare_means(aes(group = non_westernized), label = "p.format", label.x = 1.5, method = "wilcox.test") +
  #ggpubr::geom_bracket(aes(xmin = "Non_Westernized", xmax = "Westernized", y.position = 300, label = "p = <2e-16", bracket.linetype = "solid")) +
  #ggsignif::geom_signif(y_position = c(320, 340), xmin = 0.5, xmax = 1.5) +
  scale_fill_manual(values=c('#D41645','#3B6FB6')) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1)) +
  xlab("") +
  ylab("Number of unique CAZymes involved in dietary fibre metabolism") + 
  theme(axis.text.x = element_blank()) +
  theme(legend.position = "none")

median_order <- CAZyme_count_DF_df %>% group_by(study_name_info_n) %>% summarise(Median = median(Unique_CAZyme_count)) %>% arrange(-Median)

Extended_Data_Fig3_3 <- ggplot(CAZyme_count_DF_df %>% mutate(study_name_info_n = as.factor(study_name_info_n)) %>% mutate(study_name_info_n = fct_relevel(study_name_info_n, median_order$study_name_info_n))) +
  #theme_classic() +
  theme_publication() +
  aes(x = study_name_info_n, y = Unique_CAZyme_count, fill = non_westernized) +
  geom_boxplot(outlier.alpha = 0.2) +
...
Extended_Data_Fig3 <- Extended_Data_Fig3_1 + Extended_Data_Fig3_2 + Extended_Data_Fig3_3 + plot_layout(design = layout_matrix)

ggsave(here("figures/Extended_Data_Fig3_Unique_CAZymes.pdf"), Extended_Data_Fig3, width = 180, height = 150, unit = "mm")
...
ggsave(here("figures/Fig3C.pdf"), Fig_3C, width = 35, height = 45, unit = "mm")
...
ggsave(here("figures/Fig3D.pdf"), Fig_3D, width = 35, height = 45, unit = "mm")
```
...
ggsave(here("figures/Fig3E.pdf"), Fig3E, width = 130, height = 80, unit = "mm")
```
...
CAZyme_count_df_925_study <- CAZyme_count_df_925 %>% dplyr::count(study_name_W_NW_info) %>% rename("n_align" = n)
...
Extended_Data_Fig4 <- Extended_Data_Fig_4 + Extended_Data_Fig_5_1 + Extended_Data_Fig_5_2 + plot_layout(design = layout_matrix)
ggsave(here("figures/Extended_Data_Fig4_Alignment_Rates.pdf"), Extended_Data_Fig4, width = 180, height = 150, unit = "mm")
